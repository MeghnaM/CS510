...
(defun clonedGameState (currentstate)
  (if (eq '() currentstate)
    '()
    (if (not (listp (car currentstate)))
      (cons (car currentstate) (clonedGameState (cdr currentstate)))
      (cons (clonedGameState (car currentstate)) (clonedGameState (cdr currentstate))))))
;; Test:
;; (setf test (loadGameState "SBP-level0.txt"))
;; (print test)
;; (format t "~%")
;; (setf magic (clonedGameState test))
;; (setf test (applyMoveCloning test (list 2 "left")))
;; (print test)
;; (format t "~%")
;; (print magic)
